...
load_legacy_runtime_env() {
  ...
  while IFS= read -r line; do
    ...
    value="${line#*=}"
    case "${key}" in
      OPENAI_API_KEY|OPENAI_API_BASE|CODEX_API_BASE|HTTP_PROXY|HTTPS_PROXY|NO_PROXY)
        if [[ -z "${!key:-}" ]]; then
          export "${key}=${value}"
        fi
        ;;
...
refresh_container_bootstrap() {
  local container_name="$1" api_key="$2" api_base="$3" default_project="$4"
  local -a exec_args
...
wait_for_http_ready() {
  local label="$1" url="$2" attempts="${3:-60}" attempt status_code
  for ((attempt=1; attempt<=attempts; attempt+=1)); do
...
wait_for_container_http_ready() {
  local label="$1" container_name="$2" url="$3" attempts="${4:-60}" attempt status_code
  for ((attempt=1; attempt<=attempts; attempt+=1)); do
    status_code="$(
...
create_container_action() {
  ...
  local docker_mode docker_mode_input network_name volume_name agents_volume_name docker_volume_name
  local api_key api_base default_api_base
  local -a docker_args
  ...
  api_key="$(prompt_secret_default '请输入 OPENAI_API_KEY（留空表示沿用当前值或不预置）' "${OPENAI_API_KEY:-}")"
  default_api_base="${OPENAI_API_BASE:-${CODEX_API_BASE:-$(state_get_or_default 'LAST_API_BASE' '')}}"
  ...
  run_step "${action}" "refresh_bootstrap" refresh_container_bootstrap "${container_name}" "${api_key}" "${api_base}" "/workspace"
...
deploy_project_action() {
  ...
  run_step "${action}" "verify_web" wait_for_container_http_ready "CodexManager Web" "${stack_name}-web" "${web_probe_url}" 120
  run_step "${action}" "verify_admin" wait_for_container_http_ready "CodexManager Admin" "${stack_name}-server" "${admin_probe_url}" 120
  finish_action "${action}"
